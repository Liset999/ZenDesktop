...
Allows customizing the taskbar background transparency behavior. 
You can choose to style the taskbar globally or dynamically **only when there is a maximized or fullscreen window** on the active monitor.

Designed to work cleanly alongside the "Windows 11 Taskbar Styler" mod.
*/
// ==/WindhawkModReadme==
...
- backgroundStyle: blur
  ...
  $name:zh-CN: 背景风格
  $description: "The background style of the taskbar. Options: blur, acrylicBlur, color."
  $description:zh-CN: "任务栏的背景风格。可选：blur (模糊), acrylicBlur (亚克力模糊), color (纯色)。"
  $options:
    - blur: Blur
...
- color.accentColor: false
  $name: Color: Use System Accent Color
  $name:zh-CN: 颜色：使用系统主题色
- color.transparency: 255
  $name: Color: Transparency (0-255)
  $name:zh-CN: 颜色：透明度 (0-255)
- onlyWhenMaximized: false
  $name: Apply Only When Maximized
  $name:zh-CN: 仅在有窗口最大化时应用
  $description: "Only style the taskbar when there is a maximized or fullscreen window on the monitor."
  $description:zh-CN: "仅当该显示器上有最大化或全屏窗口时才应用背景样式，否则保持透明。"
- excludedPrograms: []
  $name: Excluded Programs
  $name:zh-CN: 排除的程序列表
  $description: "List of executable filenames or AppIDs to ignore for maximization checks."
  $description:zh-CN: "在此列表中的程序（如特定全屏游戏或应用）的最大化状态不会触发任务栏背景变化。"
- styleForDarkMode.use: false
  $name: Use Custom Dark Mode Style
  $name:zh-CN: 使用独立的深色模式样式
- styleForDarkMode.backgroundStyle: blur
  $name: Dark Mode Background Style
  $name:zh-CN: 深色模式背景风格
- styleForDarkMode.color.red: 0
  $name: Dark Mode Color: Red
  $name:zh-CN: 深色模式颜色：红
- styleForDarkMode.color.green: 0
  $name: Dark Mode Color: Green
  $name:zh-CN: 深色模式颜色：绿
- styleForDarkMode.color.blue: 0
  $name: Dark Mode Color: Blue
  $name:zh-CN: 深色模式颜色：蓝
- styleForDarkMode.color.transparency: 255
  $name: Dark Mode Color: Transparency
  $name:zh-CN: 深色模式颜色：透明度
*/
// ==/WindhawkModSettings==
...
HWND FindCurrentProcessTaskbarWnd() {
    ...
    EnumWindows(
        FindCurrentProcessTaskbarWndProc,
...
static BOOL CALLBACK FindTaskbarWindowsEnumProc(HWND hWnd, LPARAM lParam) {
    auto& secondaryTaskbarWindows = *reinterpret_cast<std::unordered_set<HWND>*>(lParam);
    WCHAR szClassName[32];
...
bool IsWindowExcluded(HWND hWnd) {
    ...
    LCMapStringEx(LOCALE_NAME_USER_DEFAULT, LCMAP_UPPERCASE, appId.data(),
                  static_cast<int>(appId.length()), appId.data(), static_cast<int>(appId.length()), nullptr,
                  nullptr, 0);
...
static BOOL CALLBACK PopulateMonitorStateEnumProc(HWND hWnd, LPARAM lParam) {
    ...
    if (!IsWindowActiveCandidate(hWnd, param.dwTaskbarThreadId, param.hShellWindow)) {
        return TRUE;
    }
