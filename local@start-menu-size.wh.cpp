...
Allows you to set a custom width and height for both the Windows 11 Start Menu and the Search Menu.

### Features:
- **Custom Dimensions**: Resize the Windows 11 Start Menu container grid width and height.
- **Search Pane Parity**: Applies corresponding custom sizing parameters to the taskbar Search pop-up panel.
- **XAML Visual Tree Walk**: Safely walks the UI elements and overrides properties like `Width` / `Height` / `Margin` dynamically.
- **DPI Aware Sizing**: Auto-scales the Search pane dimensions based on active monitor scale settings (DPI).
*/
// ==/WindhawkModReadme==
...
/*
- width: 0
  $name: Start Menu Width (px)
  $name:zh-CN: 开始菜单宽度 (像素)
  $description: "Set a custom width for the Start menu. 0 = default."
  $description:zh-CN: "为开始菜单设置自定义宽度。0 为默认值。"
- height: 0
  $name: Start Menu Height (px)
  $name:zh-CN: 开始菜单高度 (像素)
  $description: "Set a custom height for the Start menu. 0 = default."
  $description:zh-CN: "为开始菜单设置自定义高度。0 为默认值。"
- searchWidth: 0
  $name: Search Menu Width (px)
  $name:zh-CN: 搜索窗口宽度 (像素)
  $description: "Set a custom width for the search menu. -1 = system default, 0 = same as Start menu."
  $description:zh-CN: "为搜索窗口设置自定义宽度。-1 为系统默认，0 为与开始菜单相同。"
- searchHeight: 0
  $name: Search Menu Height (px)
  $name:zh-CN: 搜索窗口高度 (像素)
  $description: "Set a custom height for the search menu. -1 = system default, 0 = same as Start menu."
  $description:zh-CN: "为搜索窗口设置自定义高度。-1 为系统默认，0 为与开始菜单相同。"
*/
// ==/WindhawkModSettings==
...
static const UINT g_runFromWindowThreadRegisteredMsg =
...
static BOOL CALLBACK GetCoreWndEnumWindowsProc(HWND hWnd, LPARAM lParam) {
...
static void WINAPI InitFromWindowThread(PVOID) {
    StartMenuUI::Init();
}

static void WINAPI UninitFromWindowThread(PVOID) {
    StartMenuUI::Uninit();
}

static void WINAPI SettingsChangedFromWindowThread(PVOID) {
...
void Wh_ModAfterInit() {
    ...
    if (g_target == Target::StartMenu) {
        ...
        if (hCoreWnd) {
            ...
            RunFromWindowThread(
                hCoreWnd, InitFromWindowThread, nullptr);
        }
    }
}
...
void Wh_ModBeforeUninit() {
    ...
    if (g_target == Target::StartMenu) {
        ...
        if (hCoreWnd) {
            ...
            RunFromWindowThread(
                hCoreWnd, UninitFromWindowThread, nullptr);
        }
    }
}
...
void Wh_ModSettingsChanged() {
    ...
    if (g_target == Target::StartMenu) {
        ...
        if (hCoreWnd) {
            ...
            RunFromWindowThread(
                hCoreWnd, SettingsChangedFromWindowThread,
                nullptr);
        }
    }
}
