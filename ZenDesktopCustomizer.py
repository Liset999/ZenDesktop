...
class ZenCustomizerApp(ctk.CTk):
    ...
    def load_all_settings(self):
        for mod_key, path in self.mods.items():
            ...
            # 2. Blur Level
            if hasattr(self, f"blur_slider_{mod_key}"):
                blur_preset = self.get_reg_value(path, keys["blur"], defaults["blur"])
            ...
            # 3. Opacity Level
            op_preset = self.get_reg_value(path, keys["opacity"], defaults["opacity"])
            getattr(self, f"op_slider_{mod_key}").set(op_preset)
            getattr(self, f"lbl_op_val_{mod_key}").configure(text=f"{int(op_preset)}%")

            # 4. Luminosity Level
            if hasattr(self, f"lum_slider_{mod_key}"):
                lum_preset = self.get_reg_value(path, keys["luminosity"], defaults["luminosity"])
