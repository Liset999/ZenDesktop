...
* ⚠️**资源管理器透明**：为了在新版文件资源管理器中实现 WinUI 部分的半透明，请配合使用 `Windows 11 File Explorer Styler` 插件并选择其中的 `Translucent Explorer11` 主题。⚠️

* ❗**系统颜色更改**：新版系统颜色设置会调整系统的全局颜色以配合半透明或自定义主题渲染。由于这是通过全局 `SetSysColors` API 实现的，所以会无视进程排除规则。修改系统全局颜色极具挑战性，详细讨论可参考 SecureUxTheme 项目的 issue 讨论：https://github.com/namazso/SecureUxTheme/issues/9#issuecomment-611897882 ❗

* ⚠️**还原排除进程的颜色**：请在插件的“进程规则”中为目标进程添加一条规则，并禁用“Windows 主题自定义渲染”，这将尽可能为该进程还原默认的系统颜色。⚠️

* ❗**文本易读性**：Windows 自定义主题渲染会通过恢复 alpha 通道并调整文本颜色来修复不可见文字。但在某些情况下，如果将透明效果扩展到整个窗口，可能会导致文字难以阅读甚至隐形（如启用 HDR、10位色深输出、使用纯黑窗口或窗口后有纯白背景时），因为大多数 GDI 渲染操作不保留 Alpha 透明度值。❗

* ⚠️**属性窗口背景**：应用自定义渲染后，属性窗口的背景可能不会立即改变。请重启 `explorer.exe` 以刷新背景色。⚠️

* ⚠️**启用半透明效果的系统前提条件**⚠️
    - 必须启用系统设置中的“透明效果”
    - 必须禁用系统“节能模式”
#
* ⚠️本插件的背景效果对大多数现代 UWP/WinUI 窗口、使用非系统前端渲染（如 Qt、Electron、Chromium/Chrome 架构）的应用程序以及硬编码了背景颜色的窗口无效。⚠️

* ⚠️**停用后系统颜色未恢复**：如果在关闭此插件后发现系统部分 UI 颜色仍保持修改状态，这是因为自定义主题应用了新的系统颜色。只需将 Windows 系统主题切换为默认主题，然后再切回来即可解决。若仍无法解决，可作为最后手段删除注册表项 `HKEY_CURRENT_USER\Control Panel\Colors` 并重启电脑。⚠️

* ❕当窗口最大化或贴边贴靠时，模糊（Blur）效果的边缘可能会出现“像素溢出/拖影”现象，这是由系统底层的 AccentBlur API 限制所致。❕

* ✨建议配合使用自定义的黑色/暗色系统主题，例如 Rectify11 的 “Dark theme with Mica”。✨
...
- RenderingMod:
    - ThemeBackground: TRUE
      ...
      $description: >-
       使用 Direct2D 图形 API 修改 Windows 主题的部分内容，并通过修复 Alpha 通道和调整文本颜色来优化 Windows GDI 文本渲染。
        ✨建议与背景半透明效果同时启用。
    - SysColors: TRUE
      $name: 新系统颜色
      $description: >-
       通过调用 SetSysColors API 修改额外的系统 UI 颜色。
        ⚠️如遇到排除的进程出现问题，请使用 Mod 设置中的进程规则。更多详情请参阅 FAQ。
         ✨建议与 Windows 主题自定义渲染同时启用。
    - AccentColorControls: TRUE
      $name: Windows 主题强调色着色器
      $description: >-
       使用系统强调色涂装部分 Windows 主题。（需要启用 Windows 主题自定义渲染）
  $name: 渲染定制
- type: acrylicblur
  $name: 背景半透明效果
  $description: >-
     系统背景效果 (SystemBackdrop) 需要 Windows 11 版本 >= 22621.xxx (22H2)。
  $options:
  - none: 默认
...
- FlyoutsEffects: TRUE
  $name: 弹出菜单/悬浮窗效果
  $description: >-
    将半透明效果扩展到 Win32 弹出窗口（右键菜单、下拉菜单、工具提示等）。
     ✨建议同时启用背景半透明效果和 Windows 主题自定义渲染。
- ImmersiveDarkTitle: TRUE
  $name: 沉浸式暗色模式标题栏
  $description: >-
    影响系统背景效果 (SystemBackdrop) 的色调颜色，以及标题栏窗口控制按钮的图标颜色。
     ✨建议与背景半透明效果同时启用。
- ExtendFrame: TRUE
  $name: 将效果扩展到整个窗口
  $description: >-
    使用 DwmExtendFrameIntoClientArea 将半透明效果扩展到整个窗口背景。
     ✨建议与背景半透明效果同时启用。
...
- RainbowSpeed: 1
  $name: 彩虹渐变速度
  $description: 调节彩虹颜色渐变的刷新速度（取值范围：1 至 100）。
- TitlebarColor:
    - ColorTitlebar: FALSE
      $name: 启用
    - RainbowTitlebar: FALSE
      $name: 彩虹标题栏
      $description: 在窗口标题栏启用彩虹渐变效果。
    - titlerbarstyles_active: "FF0000"
      $name: 活动窗口颜色
      $description: >-
       十六进制 RGB 格式的颜色，例如 红色 = FF0000
        系统强调色 = 2
    - titlerbarstyles_inactive: "00FFFF"
      ...
      $description: >-
       十六进制 RGB 格式的颜色，例如 红色 = FF0000
        系统强调色 = 2
  $name: 标题栏颜色
  $description: 需要 Windows 11 版本 >= 22000.xxx (21H2)。会覆盖半透明效果的设置。
- TitlebarTextColor:
    - ColorTitlebarText: FALSE
      $name: 启用
    - RainbowTextColor: FALSE
      $name: 彩虹标题栏文本
      $description: 在窗口标题栏文本启用彩虹渐变效果。
  ...
  $description: >-
     需要 Windows 11 版本 >= 22000.xxx (21H2)。
      注意：此设置仅影响 Win32 窗口。由于自 Win11 22H2 起，文件资源管理器将标题栏文本渲染更改为 DirectWrite API，如需修改文件资源管理器的标题栏文本颜色，请使用 Windhawk 的 File Explorer Styler 插件。
- BorderColor:
    - ColorBorder: FALSE
      $name: 启用
    - RainbowBorder: FALSE
      $name: 彩虹边框
      $description: 在窗口边框启用彩虹渐变效果。
